		...
		try {
			if (evt.getSource().equals(view.getPesquisar())) {
				if (view.getPesquisa().getText().isEmpty())
					...
					String pesquisa = " WHERE "+ view.getOpcoes().getSelectedItem().toString()+ " ";
					if(conteudo.contains("/")){
						System.out.println("contem");
						String data[] = conteudo.split("/");
						if(data.length==3)conteudo=data[2]+"-"+data[1]+"-"+data[0];
						else conteudo=data[1]+"-"+data[0];
					} 
					
					if (view.getTipo().isSelected())
						pesquisa = pesquisa+ "= '" + conteudo+ "'";
					else pesquisa = pesquisa+ "LIKE '%" + conteudo+ "%'";
				...
				if (evt.getSource().equals(view.getRemover())) {
					if (JOptionPane.showConfirmDialog(null,
							"Deseja mesmo remover o item selecionado?","Remover Item", 0) == 0) {
						String aux = "Não";
						if (remover(i)) {
