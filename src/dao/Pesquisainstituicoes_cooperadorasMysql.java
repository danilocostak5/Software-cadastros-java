...
public void inserir(long id, ArrayList<InstituicaoCooperadora> lista) throws Exception{
for(InstituicaoCooperadora x:lista){
String sql = "INSERT INTO Pesquisainstituicoes_cooperadoras (id1,id2) VALUES("+id+","+x.getId()+")";
try {
...
String sql = "DELETE FROM Pesquisainstituicoes_cooperadoras WHERE id1 = "+id;
try {
comando.execute(sql.toString());
} catch (SQLException e) {
throw e;
...
if(id>0){
String sql = "SELECT * FROM Pesquisainstituicoes_cooperadoras WHERE id1 = "+id;
try {
ResultSet rs = comando.executeQuery(sql);
while (rs.next()) {
InstituicaoCooperadora x = new InstituicaoCooperadoraMysql().listar(" WHERE id="+(rs.getLong("id2"))).get(0);
lista.add(x);
