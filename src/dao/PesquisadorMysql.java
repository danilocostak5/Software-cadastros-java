...
"(nome,nome_cientifico,email,sexo,classe,titulacao,curso_vinculado,areaformacao) VALUES('" 
	+ pesquisador.getNome() + "','"+ pesquisador.getNome_cientifico() + "','"+ pesquisador.getEmail() + "','"+ pesquisador.getSexo() + "','"+ pesquisador.getClasse() + "','"+ pesquisador.getTitulacao() + "','"+ pesquisador.getCurso_vinculado().getId() + "','"+ pesquisador.getAreaformacao().getId() + "')";
long x=0;
try {
...
+"nome = '"+pesquisador.getNome()+"',"+"nome_cientifico = '"+pesquisador.getNome_cientifico()+"',"+"email = '"+pesquisador.getEmail()+"',"+"sexo = '"+pesquisador.getSexo()+"',"+"classe = '"+pesquisador.getClasse()+"',"+"titulacao = '"+pesquisador.getTitulacao()+"',"+"curso_vinculado = '"+pesquisador.getCurso_vinculado().getId()+"',"+"areaformacao = '"+pesquisador.getAreaformacao().getId()+"'"
+" WHERE id = '"+x +"'";
...
try {
ResultSet rs = comando.executeQuery("SELECT * FROM pesquisador "+condicao);
while (rs.next()) {
...
//pesquisador.setCurso_vinculado(new CursoMysql().listar(" WHERE id ="+(rs.getLong("curso_vinculado"))).get(0));
pesquisador.setAreaformacao(new AreaFormacao(rs.getLong("areaformacao")));
//pesquisador.setAreaformacao(new AreaFormacaoMysql().listar(" WHERE id ="+(rs.getLong("areaformacao"))).get(0));
